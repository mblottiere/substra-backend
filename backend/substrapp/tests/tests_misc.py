...
class MiscTests(TestCase):
    ...
    def test_get_object_from_ledger(self):
        with patch('substrapp.ledger_utils.query_ledger') as mquery_ledger:
            ...
            self.assertRaises(LedgerNotFound, get_object_from_ledger, 'mychannel', 'pk', 'fake_query')

        with patch('substrapp.ledger_utils.query_ledger') as mquery_ledger:
            mquery_ledger.side_effect = LedgerInvalidResponse('Bad Response')
            self.assertRaises(LedgerInvalidResponse, get_object_from_ledger, 'mychannel', 'pk', 'fake_query')

        with patch('substrapp.ledger_utils.query_ledger') as mquery_ledger:
            mquery_ledger.return_value = {'key': 'pk'}
            data = get_object_from_ledger('mychannel', 'pk', 'good_query')
    ...
    def test_log_fail_tuple(self):
        with patch('substrapp.ledger_utils.update_ledger') as mupdate_ledger:
            ...
            log_fail_tuple('mychannel', 'traintuple', 'pk', 'error_msg')

        with patch('substrapp.ledger_utils.update_ledger') as mupdate_ledger:
            mupdate_ledger.return_value = None
            log_fail_tuple('mychannel', 'testtuple', 'pk', 'error_msg')

    def test_log_start_tuple(self):
        with patch('substrapp.ledger_utils.update_ledger') as mupdate_ledger:
            mupdate_ledger.return_value = None
            log_start_tuple('mychannel', 'traintuple', 'pk')

        with patch('substrapp.ledger_utils.update_ledger') as mupdate_ledger:
            mupdate_ledger.return_value = None
            log_start_tuple('mychannel', 'testtuple', 'pk')
    ...
    def test_log_success_tuple(self):
        with patch('substrapp.ledger_utils.update_ledger') as mupdate_ledger:
            ...
            log_success_tuple('mychannel', 'traintuple', 'pk', res)
        ...
        with patch('substrapp.ledger_utils.update_ledger') as mupdate_ledger:
            ...
            log_success_tuple('mychannel', 'testtuple', 'pk', res)

    def test_query_tuples(self):
        with patch('substrapp.ledger_utils.query_ledger') as mquery_ledger:
            mquery_ledger.return_value = None
            query_tuples('mychannel', 'traintuple', 'data_owner')

        with patch('substrapp.ledger_utils.query_ledger') as mquery_ledger:
            mquery_ledger.return_value = None
            query_tuples('mychannel', 'testtuple', 'data_owner')
