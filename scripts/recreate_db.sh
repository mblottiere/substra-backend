...
dropdb -U ${USER} backend_org4
createdb -U ${USER} -E UTF8 backend_org4
psql -U ${USER} -d backend_org4 -c "GRANT ALL PRIVILEGES ON DATABASE backend_org4 to backend;ALTER ROLE backend WITH SUPERUSER CREATEROLE CREATEDB;"
